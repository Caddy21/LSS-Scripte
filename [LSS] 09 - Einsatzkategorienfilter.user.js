...
(function () {
    ...
    // Hole alten Zustand vor dem Entfernen
    ...
    if (missions.length === 0 && previousMissions.length > 0) {
        console.log("[CustomAlarm] Letzter gespeicherter Einsatz wurde entfernt.");
        alert("Dies ist der letzte Einsatz in der ausgewählten Kategorie/Gruppe.");
        return;
    }

    if (missions.length === 0) {
        console.log("[CustomAlarm] Keine gespeicherten Einsätze vorhanden – keine Weiterleitung.");
        return;
    }
    ...
    const hasWarningIcon = !!doc.querySelector('.mission_header_info.row img[src="/images/fire_rot.png"]');
    if (hasWarningIcon) {
        console.log("[CustomAlarm] Warnsymbol gefunden – Weiterleitung unterdrückt.");
        return;
    }
    ...
    if (drivingOwn || atSceneOwn) {
        console.log("[CustomAlarm] Eigene Fahrzeuge bereits alarmiert oder vor Ort – direkt zum nächsten Einsatz.");
        iframe.src = `https://www.leitstellenspiel.de/missions/${nextId}`;
    } else {
        console.log("[CustomAlarm] Beobachte Alarmieren-Klick.");
        alarmBtn.addEventListener('click', () => {
            const recheckWarningIcon = !!doc.querySelector('.mission_header_info.row img[src="/images/fire_rot.png"]');
            if (recheckWarningIcon) {
                console.log("[CustomAlarm] Warnsymbol nach dem Alarmieren vorhanden – Weiterleitung abgebrochen.");
                return;
            }
            console.log("[CustomAlarm] Du hast alarmiert – Weiterleitung zum nächsten Einsatz.");
            iframe.src = `https://www.leitstellenspiel.de/missions/${nextId}`;
        }, { once: true });
    }
}
