// ==UserScript==
// @name         [LSS] Auszeichnungsverwalter
...
(function() {
    ...
    const observer = new MutationObserver(() => {
        ...
        if (iframe && !iframe.hasAttribute('data-processed')) { // Verhindern, dass der iFrame mehrfach verarbeitet wird
            console.log("[Tampermonkey] iFrame gefunden:", iframe);
            iframe.onload = function() {
                console.log("[Tampermonkey] iFrame vollständig geladen.");
            ...
            console.log("[Tampermonkey] Kein iFrame oder bereits verarbeitet.");
        }
    });
    ...
    function processIframe(iframe) {
        ...
        if (iframeDoc.readyState === "complete") {
            console.log("[Tampermonkey] iFrame-Dokument vollständig geladen.");
            addTabs(iframeDoc); // Tabs hinzufügen
        } else {
            iframe.onload = function() {
                console.log("[Tampermonkey] iFrame-Dokument vollständig geladen (onload).");
    ...
    function addTabs(iframeDoc) {
        console.log("[Tampermonkey] Beginne mit Tab-Erstellung...");
        ...
        if (headers.length === 0) {
            console.warn("[Tampermonkey] Keine Auszeichnungen im iFrame gefunden.");
        }

        headers.forEach(header => {
            let text = header.innerText.trim();
            console.log(`[Tampermonkey] Gefundener Text in Auszeichnung: ${text}`);

            // Überprüfen, ob eine Jahreszahl vorhanden ist
            let yearMatch = text.match(/\b(20\d{2})\b/); // Jahreszahlen suchen
            if (yearMatch) {
                years.add(yearMatch[1]);
                console.log(`[Tampermonkey] Jahreszahl gefunden: ${yearMatch[1]}`);
            } else if (text.includes("VIP")) {
                vipItems.push(header.closest(".grid-item"));
                console.log(`[Tampermonkey] "VIP" Auszeichnung gefunden, wird unter "VIP" angezeigt.`);
            } else {
        ...
        console.log(`[Tampermonkey] Erkannte Jahreszahlen: ${Array.from(years).join(', ')}`);
        console.log(`[Tampermonkey] "Allgemein"-Auszeichnungen: ${generalItems.length}`);
        console.log(`[Tampermonkey] "VIP"-Auszeichnungen: ${vipItems.length}`);
        ...
        if (!tabContainer) {
            ...
            if (header) {
                ...
                console.warn("[Tampermonkey] Kein Header gefunden, Tabs wurden nicht eingefügt.");
            }
            ...
            console.log("[Tampermonkey] Tab-Container existiert bereits.");
        }
    }
})();
